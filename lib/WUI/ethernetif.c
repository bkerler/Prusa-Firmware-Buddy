...
static void LAN8742_StartAutoNego(void) {
    uint32_t regvalue = 0;
    if (HAL_ETH_ReadPHYRegister(&heth, LAN8742_BCR, &regvalue) == HAL_OK) {
        regvalue |= LAN8742_BCR_AUTONEGO_EN;
        HAL_ETH_WritePHYRegister(&heth, LAN8742_BCR, regvalue);
    }
}

static void LAN8742_SetLinkState(void) {
    uint32_t bcrvalue = 0;
    if (HAL_ETH_ReadPHYRegister(&heth, LAN8742_BCR, &bcrvalue) == HAL_OK) {
...
static void LAN8742_SetSpeedAndMode(void) {
    ...
    {
        if (HAL_ETH_ReadPHYRegister(&heth, LAN8742_PHYSCSR, &regvalue) != HAL_OK) {
            goto error;
        }
