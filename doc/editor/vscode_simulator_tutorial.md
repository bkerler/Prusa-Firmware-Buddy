...
1. ```bash
   cd ~ && sudo apt install git python3.10-venv python3-pip -y
   ```
1. ```bash
...
## Install and update MINI404 simulator
```bash
git clone https://github.com/vintagepc/MINI404 --recursive
sudo su
...
cd MINI404
./configure --target-list=buddy-softmmu,xtensa-softmmu --enable-gtk --enable-libusb --disable-werror
make -j`nproc`
cd build
cp qemu-system-xtensa qemu-system-buddy ~/Prusa-Firmware-Buddy/.dependencies/mini404-0.9.10/
cp -r pc-bios ~/Prusa-Firmware-Buddy/.dependencies/mini404-0.9.10/
cd ..
cp -r hw/arm/prusa/assets/mini ~/Prusa-Firmware-Buddy/.dependencies/mini404-0.9.10/
```
...
3. This step is only needed if necessary under 24.04
    ```bash
...
## Run tests
```bash
mkdir build-test ; cd build-test ; cmake .. -DBOARD=XBUDDY; make tests -j`nproc`
```

## Compile via command line
```bash
python ./utils/build.py --preset coreone --build-type release
```
